...
export function ExerciseCard({ exercise }: { exercise: ExerciseWithCategory }) {
  const media = resolveMedia(exercise.video_url, exercise.thumbnail_url)
  const thumb = media?.thumb ?? null
  const color = exercise.parent_category?.color ?? exercise.category?.color ?? null
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--card)] px-4 py-3"
      ...
      <div className="w-24 h-14 rounded-md overflow-hidden bg-[var(--muted)] flex-shrink-0 relative">
        {media ? (
          <MediaLauncher media={media} title={exercise.name}>
            <button className="absolute inset-0 w-full h-full cursor-pointer">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/10 hover:bg-black/25 transition-colors">
                <span className="text-white/80 text-base drop-shadow">{media.mode === 'image' ? '⤢' : '▶'}</span>
              </div>
            </button>
          </MediaLauncher>
        ) : thumb && imgOk ? (
